...
module Backlogs
  class Statistics
    def initialize(project)
      ...
      @past_sprints= RbSprint.where("project_id = ? and not(effective_date is null or sprint_start_date is null) and effective_date < ?", @project.id, Date.today).order("effective_date desc").limit(5).find_all().select(&:has_burndown?)
  ...
  module ProjectPatch
    ...
    module InstanceMethods
      ...
      def active_sprint
        ...
        #])
        @active_sprint ||= RbSprint.where("project_id = ? and status = 'open' and not (sprint_start_date is null or effective_date is null) and ? >= sprint_start_date and ? <= effective_date", self.id, time.end_of_day, time.beginning_of_day).first()
      end
